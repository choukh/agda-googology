...
| [Fusion.lagda.md](Fusion.lagda.md) | ~200 | ✅ 编译 (--safe 无公理) | **范式融合** `V : T → Set`; 证书已建到 ε₀, `f_{ε₀}(99)` 零未证义务 (§2.4) |
...
### 2.3 Fusion 模块 (Phase N-2.5) — 范式 A × 范式 B 融合体

回应作者提问 "能否保留 Brouwer 树骨架 + 有限项语法 + 无条件三歧性 + WF 反射作为必须义务":
**可以, 融合体 = 语法索引的 Brouwer 树** `V : T → Set` ([Fusion.lagda.md](Fusion.lagda.md)):

- `vω : (∀ n → V (t[n])) → V t` — Brouwer 骨架 (ℕ-分支树), FGH 结构递归, **无 Acc**;
- 三歧性在语法索引上: `cmpⱽ = cmpᵀ`, 无条件可判定 (taboo 不触碰树本身);
- **WF 反射内置为数据**: `V t` 的居民就是 t 的基本列展开证书, 无证书即无序数;
- 整体 WF(NF) 大墙分解为**逐算子 V-提升引理** (`V-++`, `V-rep`, `V-ψ0` 已证),
  实测建到 **ε₀ 证书** (`vεₒ`, 经 γ-链的定义性计算) — `f_{ε₀}(99)` 成为本仓库第一个
  "三歧 + 结构停机 + 零未证义务" 同时成立的大数;
- 阶梯上行 = 逐条新 V-提升 (Ω-混合参数 → 一般 ψ-提升 → vΩ-证书 → Mahlo 段接 Takahashi-𝕄),
  见 Fusion §6. 天花板从 "结构 taboo" 变为 "证明工程" — 无有限终点但每段有限.

### 2.4 诚实边界 (EBO/Mahlo 共享, 细节见各文件 §8)
...
- [Fusion.lagda.md](Fusion.lagda.md) — Phase N-2.5: 范式融合 V : T → Set, 证书至 ε₀ (~200 LOC, ✅)
- 本文件 — umbrella
