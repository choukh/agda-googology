...
类比 [Ordᴰ](../BTBO.lagda.md#L639) 的 `lim : (f : ℕ → Ordᴰ) (mono : monotonic f) → Ordᴰ` 直接对偶到 Ord₁ 的 lim₁: `lim₁ : (f : Ord₀ → Ord₁ᴰ) (mono₁ : monotonic₁ f) → Ord₁ᴰ`, 其中 `monotonic₁ f = ∀ {a b : Ord₀} → a <₀ b → f a <₁ f b`.
...
BTBO 的 [Ord-Basic 模块](../BTBO.lagda.md#L85) 只给了 Ord₀ 的 data 定义, 没定义 < 关系. 这里补上 (类似 Ordᴰ 的 < 但无 mono 字段):
...
**实测**: Agda 接受这个 induction-induction 定义, positivity 检查通过. 与 [BTBO Ordᴰ](../BTBO.lagda.md#L643-L662) 结构同形, 多一个 lim₁ 构造子.

## 3. `<₁-trans` 传递性

类比 [BTBO `<-trans`](../BTBO.lagda.md#L681) (对 q 归纳):
