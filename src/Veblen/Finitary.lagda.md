...
**定义** 对函数 $F : A^{→n}$ 的参数全部填零所得到的 $A$ 的元素, 记作 $F\kern{0.17em}\overset{.}{0} : A$, 递归定义为
...
**定义** 对函数 $F : A^{→n^+}$ 的参数留最后一位不填, 其余全部填零, 所得到的函数 $\text{Ord}→A$, 记作 $F\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}}$, 递归定义为

$$
\begin{aligned}
(F : A^{→1})\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}} &= F:\text{Ord}→A \\
(F : A^{→n^{++}})\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}} &= (F\kern{0.17em}0 : A^{→n^+})\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}}
\end{aligned}
$$
...
**定义** 对函数 $F : A^{→n^{++}}$ 的参数留最后两位不填, 其余全部填零, 所得到的函数 $\text{Ord}→\text{Ord}→A$, 记作 $F\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}}\kern{0.17em}\underbar{\kern{0.5em}}$, 递归定义为

$$
\begin{aligned}
(F : A^{→2})\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}}\kern{0.17em}\underbar{\kern{0.5em}} &= F:\text{Ord}→\text{Ord}→A \\
(F : A^{→n^{+++}})\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}}\kern{0.17em}\underbar{\kern{0.5em}} &= (F\kern{0.17em}0 : A^{→n^{++}})\kern{0.17em}\overset{.}{0}\kern{0.17em}\underbar{\kern{0.5em}}\kern{0.17em}\underbar{\kern{0.5em}}
\end{aligned}
$$
...
```agda
Φ-ż-s-x : Φ {n} (Φⁿ F) 0̇, (suc α) ,_ ≡ fixpt (Φ {n} (Φⁿ F) 0̇, α ,_)
Φ-ż-s-x {n = zero} = refl
Φ-ż-s-x {n = suc n} = Φ-ż-s-x {n}

Φ-ż-l-x : Φ {n} (Φⁿ F) 0̇, (lim f) ,_ ≡ jump λ β → lim λ m → Φ {n} (Φⁿ F) 0̇, (f m) , β
